...
btnImagem.addEventListener("click", () => {
    html2canvas(cartao, { scale: 1.5 }).then(canvas => {
...
btnEmail.addEventListener("click", () => {
    const assunto = document.getElementById("emailCorpo").value || "Comunicado de inscrição pendente UniBB";
    const corpo = document.getElementById("descricaoImagem").value || "Segue comunicado de inscrição pendente.";
...
Um funcionário de sua dependência manifestou interesse em participar do curso "${curso}", que irá ocorrer na cidade de ${cidadeUf} de ${periodo}.

Gostariamos de verificar a sua autorização para a matrícula.

Com o seu incentivo, o time se engaja e os resultados aparecem: funcionários mais qualificados e clientes mais satisfeitos.
Ao desenvolver sua equipe, você também fortalece sua liderança e impulsiona o desempenho da dependência.

A inscrição pode ser validada no Portal Capacita Aqui, na aba "Validação Gestor/a".
